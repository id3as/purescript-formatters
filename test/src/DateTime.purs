...
import Test.Utils (forAll, makeDateTime, shouldEqual)

datetimeTest ∷ Free TestF Unit
datetimeTest =
  suite "Data.Formatter.DateTime" do
    forAll (\a → a.format <> " | " <> a.dateStr)
      ...
      , { format: "YYYY", dateStr: "0000", date: makeDateTime (0) 0 0 0 0 0 0 }
      ]
    ...
    suite "hour 24" do
      test "24 hour and more then 0 time component"
        $ do
            let
              err = (Left "When hour is 24, other time components must be 0 (line 1, col 24)")
    ...
    suite "hour {0,12} {am,pm}" do
      let
        format = "hh a"
      test "00 AM" $ FDT.unformatDateTime format "00 AM" `shouldEqual` (Right $ makeDateTime 0 1 1 0 0 0 0)
      test "00 PM" $ FDT.unformatDateTime format "00 PM" `shouldEqual` (Right $ makeDateTime 0 1 1 12 0 0 0)
      test "12 PM" $ FDT.unformatDateTime format "12 PM" `shouldEqual` (Right $ makeDateTime 0 1 1 12 0 0 0)
      test "12 AM" $ FDT.unformatDateTime format "12 AM" `shouldEqual` (Right $ makeDateTime 0 1 1 0 0 0 0)
    suite "parseFormatString" do
      forAll
        _.str
        "should parse"
        dateformats
